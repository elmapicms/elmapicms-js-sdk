...
export class ElmapiError extends Error {
  ...
  constructor(
    ...
    if (requestInfo !== undefined) this.requestInfo = requestInfo;
...
export class AuthenticationError extends ElmapiError {
  constructor(message: string = 'Authentication failed. Please check your API token.', requestInfo?: any) {
...
export class AuthorizationError extends ElmapiError {
  constructor(message: string = 'You do not have permission to perform this action.', requestInfo?: any) {
...
export class RateLimitError extends ElmapiError {
  constructor(message: string = 'Rate limit exceeded. Please try again later.', requestInfo?: any) {
...
export class ServerError extends ElmapiError {
  constructor(message: string = 'Server error occurred. Please try again later.', requestInfo?: any) {
...
export class NetworkError extends ElmapiError {
  constructor(message: string = 'Network error occurred. Please check your connection.', requestInfo?: any) {
 ...
 */
function createApiError(error: any, requestInfo: any): ElmapiError {
 ...
 */
function createNetworkError(error: any, requestInfo: any): ElmapiError {
...
export class ApiClient {
   ...
   * Get project
   */
  async getProject(params?: { with?: string }) {
   ...
   */
  async getEntries(collectionSlug: string, params?: {
   ...
   */
  async getEntry(collectionSlug: string, uuid: string) {
    try {
   ...
   * Create a new entry
   */
  async createEntry(collectionSlug: string, data: {
    locale?: string;
   ...
   * Update an entry
   */
  async updateEntry(collectionSlug: string, uuid: string, data: {
    locale?: string;
    state?: 'draft' | 'published';
    published_at?: string;
    data: object;
   ...
   * Patch an entry
   */
  async patchEntry(collectionSlug: string, uuid: string, data: {
    locale?: string;
    state?: 'draft' | 'published';
    published_at?: string;
    data: object;
   ...
   */
  async deleteEntry(collectionSlug: string, uuid: string, force?: boolean) {
   ...
   * Upload a new asset
   */
  async uploadAsset(file: File | Buffer, metadata?: any) {
    ...
    const requestInfo = {
      ...
      url,
      params: {},
      data: { file: (file as File).name || 'file', metadata },
      timeout: this.timeout
    };
   ...
   * Delete an asset
   */
  async deleteAsset(identifier: string, force?: boolean) {
    const params = force ? { force } : {};
  ...
  validateConfiguration(): void {
    ...
    if (errors.length > 0) {
      throw new ValidationError(
        `Invalid client configuration: ${errors.join(', ')}`,
        { errors }
      );
    }
  }
